<!--
  Source: https://styles.refero.design/style/e5f5f8cf-e68d-4ed1-bbf5-6b67569af648 (Refero Styles — Dala), pulled 2026-09-19.
  Everything above "SignLoop Implementation" is the upstream reference, unchanged except the truncated
  Deep Verdant hex "#15846" was corrected to "#15846e". SignLoop-specific adaptations live in the final section.
-->
...
Small triangular icon in #8052ff (violet) with a gradient fade through #15846e (teal), paired with 'Dala' wordmark in white. The icon is a stylized angular fragment — geometric, sharp-edged, echoing the triangular particles in the hero visualization.
...
Small outlined triangles in various chromatic colors (#8052ff violet, #ffb829 amber, #15846e teal, plus assorted purples and blues) scattered at low opacity across the background outside the main constellation. Creates atmospheric depth without competing with the central visualization.
...
## SignLoop Implementation

Everything above is the upstream Dala reference. This section records how SignLoop applies it, including where a dense legal workspace deviates from a marketing site. When the two disagree, this section wins for SignLoop code.

### Where the tokens live

| Layer | File | Notes |
|-------|------|-------|
| CSS variables | `apps/web/app/globals.css` | The Quick Start `:root` block verbatim (range values such as `--section-gap` split into `-min`/`-max`), then the SignLoop extensions and semantic aliases below. |
| Tailwind | `apps/web/tailwind.config.ts` | The repo runs Tailwind **v3.4**, so the v4 `@theme` block is translated into `theme.extend`. Colours go through `color-mix()` so opacity modifiers (`bg-primary/10`) work on the hex variables. |
| Class merging | `apps/web/lib/utils.ts` | `cn()` is taught the custom type scale and radii. Without it, `tailwind-merge` reads `text-caption` as a colour and drops it. |
| Clerk | `apps/web/lib/clerk-appearance.ts` | Literal hex copies of the tokens; Clerk cannot read CSS variables. |
...
PPNeueMontreal is a licensed face and is not bundled. `--font-ppneuemontreal` lists it first, then falls back to **Inter** (the documented substitute), loaded as a variable font through `next/font` so the ultra-light 200 weight is real. If a licence is obtained, register the files under the family name `PPNeueMontreal` (for example with `next/font/local`) and nothing else needs to change. `"ss01"` is enabled on `body` as specified.
...
| Token | Value | Purpose |
|-------|-------|---------|
| `--color-void-raised` | `#0c0c0c` | Menus, dialogs, toasts, the composer: the only layer allowed above the void |
| `--color-graphite` | `#171717` | Hover and selected fills, user chat bubbles, skeletons |
| `--color-hairline` | `#1f1f1f` | Structural separators (`--border`) |
| `--color-hairline-strong` | `#333333` | Form-control outlines (`--input`) |
| `--color-signal-red` | `#ff5c5c` | Destructive actions, high risk (6.9:1 on void) |
| `--color-verdant-glow` | `#3fc9a6` | Low risk and success text; Deep Verdant itself is too dark to read on black |

Semantic aliases used by components: `background`/`card` = Void, `foreground` = Bone White, `muted-foreground` = Ash Gray, `subtle-foreground` = Silver Mist, `primary`/`ring` = Electric Iris, `highlight` = Saffron Spark, `success` = Verdant Glow, `destructive` = Signal Red, `popover` = Void Raised, `secondary`/`muted`/`accent` = Graphite. `--radius` is 24px.

### Rules adapted for a working app

- **Borders:** Dala has none. SignLoop allows a `#1f1f1f` hairline for structure only: the sidebar edge, cards in list grids, form controls, overlays. Never use one for decoration. No accent rules (`border-l-4 border-l-primary`). Dashed outlines are reserved for file drop zones.
- **Cards:** 24px radius, hairline, no fill, no shadow. Interactive cards respond with `hover:border-input hover:bg-foreground/[0.03]`.
- **Buttons:** `default` is the Electric Iris pill: 14px, weight 600, uppercase, 0.025em tracking. Use one per view; secondary actions use `outline` (hairline pill) or `ghost` (text only). `destructive` is a Signal Red outline pill, never a second saturated fill.
- **Headings:** weight 400 everywhere; hierarchy comes from the size tokens. Page titles use `.app-title` (36px, 42px from `md`) under a Saffron `.app-eyebrow` label; supporting copy uses `.app-lede` (18px / 200 / Silver Mist). Dense UI text stays at 15px / 400 (`text-sm`), matching the 15px/400 body noted in the typography section above.
- **Navigation:** sidebar sections are 14px / 600 uppercase nav labels, Ash Gray at rest and white when active or hovered. The header stays transparent, with no divider.
- **Chat:** assistant replies float on the void with no bubble (`prose-dala` typography: Saffron links, Iris quote rule). User messages sit in a Graphite bubble; violet stays reserved for actions.
- **Risk semantics:** high = Signal Red, medium = Saffron Spark, low = Verdant Glow (`getRiskColor` in `lib/utils.ts`).
- **Imagery:** `components/constellation.tsx` draws the particle brain on a canvas: seeded, batched per colour and depth, paused offscreen, and static under `prefers-reduced-motion`. It appears only in the landing hero; below `lg` it becomes a 30%-opacity ambient field behind the copy.
- **Logo:** an Electric Iris triangle fragment fading to Deep Verdant, beside a white "SignLoop" wordmark (`SignLoopWordmark` in `components/dashboard.tsx`). The same mark is the only site icon: `app/icon.svg` (transparent), `app/favicon.ico` (16/32/48px renders of it) and `app/apple-icon.png` (on the void, padded). The logo and the constellation are the only gradients.

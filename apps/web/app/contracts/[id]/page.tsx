...
const ContractDetails = () => {
  ...
  const getContractErrorDetails = (error: unknown) => {
    ...
    if (typeof status === "number" && status >= 500) {
      return {
        title: "Server error",
        description:
          "We hit a server error while loading this contract. Please try again soon.",
      };
    }
    ...
    return {
      ...
      description:
        message ||
        "We could not load this contract right now. Please try again later.",
    };
  };
  ...
  const resultJson: Partial<AnalysisResult> | null =
    analysis?.resultJson && typeof analysis.resultJson === "object"
      ? analysis.resultJson
      : null;
  const summary =
    resultJson?.summary && typeof resultJson.summary === "object"
      ? resultJson.summary
      : null;
  const summaryPayments =
    summary?.payments && typeof summary.payments === "object"
      ? summary.payments
      : null;
  const summaryTerm =
    summary?.term && typeof summary.term === "object" ? summary.term : null;
  const summaryRenewal =
    summary?.renewal && typeof summary.renewal === "object"
      ? summary.renewal
      : null;

  const riskBadge =
    typeof analysis?.riskBadge === "string"
      ? analysis.riskBadge
  ...
  const summaryCancellation =
    summary?.cancellation && typeof summary.cancellation === "object"
      ...
      : null;
  const keyDates: AnalysisResult["key_dates"] = Array.isArray(
    resultJson?.key_dates,
  )
    ? resultJson.key_dates
    : [];
  const nextActions =
    resultJson?.next_actions && typeof resultJson.next_actions === "object"
  ...
  const questionsToAsk: string[] = Array.isArray(nextActions?.questions_to_ask)
    ...
    : [];
  const emailTemplates: AnalysisResult["next_actions"]["email_templates"] =
    Array.isArray(nextActions?.email_templates)
      ? nextActions.email_templates
      : [];

  const uploadedDateLabel = formatDate(
    contract?.createdAt,
    "MMM d, yyyy",
    "Unknown date",
  );
  const analysisDateLabel = formatDate(
    analysis?.createdAt,
    "MMM d, HH:mm",
    "Unknown time",
  );
  ...
  const llmProviderLabel =
    llmProvider === "openrouter"
      ? "OpenRouter"
      : llmProvider === "primary-openai-compatible"
        ? "Primary provider"
        : llmProvider === "openai"
          ? "OpenAI"
          : llmProvider;
  ...
  useEffect(() => {
    ...
    const stillExists = sortedAnalyses.some(
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This will permanently delete the contract and all
                      associated analyses. This cannot be undone.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel
          ...
          <>
            ...
            <div className="grid gap-6 md:grid-cols-3">
              ...
              <Card className="md:col-span-1 border-l-4 border-l-primary">
                ...
                <CardContent>
                  <Badge
                    className={`text-lg px-4 py-1 ${getRiskColor(riskBadge, { hover: true })}`}
                  >
                    {riskBadge || "UNKNOWN"}
                  </Badge>
                  ...
                  {!!llmModel && (
                    <p className="mt-2 text-xs text-muted-foreground">
                      LLM: {llmProviderLabel ? `${llmProviderLabel} • ` : ""}
                      {llmModel}
                    </p>
                  )}
                </CardContent>
              ...
              <Card className="md:col-span-2">
                <CardHeader>
                  ...
                  <CardDescription>
                    Important points extracted from the contract
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  ...
                  {keyFindings.length > 0 ? (
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      No key findings yet. Re-run analysis to extract
                      highlights.
                    </div>
                  )}
                </CardContent>
            ...
            <Card className="mt-6">
              <CardHeader>
                ...
                <CardDescription>
                  Quick overview of the contract terms
                </CardDescription>
              </CardHeader>
              <CardContent>
                {summary ? (
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="p-4 rounded-lg bg-muted/40">
                      ...
                      <p className="text-sm">
                        {summary.what_it_is || "Not specified"}
                      </p>
                    </div>
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    Summary unavailable. Re-run analysis to generate the
                    contract overview.
                  </div>
                )}
              </CardContent>
                ...
                <CardContent>
                  <div className="grid gap-4 md:grid-cols-3">
                    <div className="p-4 rounded-lg bg-muted/40">
                      ...
                      <p className="text-sm">
                        {summaryCancellation.how || "Not specified"}
                      </p>
                    </div>
                    <div className="p-4 rounded-lg bg-muted/40">
                      <h4 className="text-sm font-medium text-muted-foreground mb-1">
                      ...
                      <p className="text-sm">
                        {typeof summaryCancellation.notice_period_days ===
                        "number"
                    ...
                    <div className="p-4 rounded-lg bg-muted/40">
                      ...
                      {Array.isArray(summaryCancellation.penalties) &&
                      summaryCancellation.penalties.length > 0 ? (
                        <ul className="text-sm list-disc list-inside space-y-1">
                          {summaryCancellation.penalties.map(
                            (penalty: string, idx: number) => (
                ...
                <CardContent>
                  <div className="space-y-4">
                    {redFlags.map((flag, idx) => {
                      const type =
                        typeof flag?.type === "string" ? flag.type : "Risk";
                      const explanation =
                      ...
                      const severity =
                        ...
                        Number.isFinite(flag.severity)
                          ? Math.max(0, Math.min(10, flag.severity))
                          : null;
                      const where =
                        typeof flag?.where === "string" ? flag.where : "";
                    ...
                    {keyDates.map((date, idx) => (
                      ...
                      >
                        ...
                        <div>
                          ...
                          {typeof date?.derived_from === "string" &&
                            date.derived_from && (
                              <p className="text-xs text-muted-foreground italic">
                                From: {date.derived_from}
                              </p>
                            )}
                        </div>
                      </div>
            ...
            {(questionsToAsk.length > 0 || emailTemplates.length > 0) && (
              <Card className="mt-6">
                <CardHeader>
                  ...
                  <CardDescription>
                    Suggested follow-up based on the analysis
                  </CardDescription>
                </CardHeader>
                <CardContent>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <div>
                ...
                <CardDescription>
                  Manage older analyses for this contract
                </CardDescription>
              </div>
              <Button
                variant="destructive"
                ...
                {olderAnalyses.map((a) => (
                  ...
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <Badge
                          className={getRiskColor(a.riskBadge, { hover: true })}
                        >
                          {a.riskBadge || "UNKNOWN"}
                        </Badge>
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        variant={
                          analysis?.id === a.id ? "secondary" : "outline"
                        }
                        size="sm"
